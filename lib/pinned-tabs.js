...
class PinnedTabs {
  ...
  setObservers() {
    ...
    // Prevent destroying pinned tabs if needed
    this.subscriptions.add(
      atom.workspace.onWillDestroyPaneItem(({item, pane, prevent}) => {
        if (!atom.config.get('pinned-tabs.preventCloseAll')) return;

        // Verify that all tabs are being closed and not just this one
        if (!this.willCloseAllTabs) return;
    ...
    atom.commands.onWillDispatch(e => {
      if (e.type === 'tabs:close-all-tabs') this.willCloseAllTabs = true;
    });
    atom.commands.onDidDispatch(e => {
      if (e.type === 'tabs:close-all-tabs') this.willCloseAllTabs = false;
    });
  }
