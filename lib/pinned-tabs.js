...
class PinnedTabs {
  ...
  setObservers() {
    ...
    // Prevent destroying pinned tabs in batch
    this.subscriptions.add(
      atom.workspace.onWillDestroyPaneItem(({item, pane, prevent}) => {
        // Check if the user wants to prevent destroying pinned tabs in batch
        if (!atom.config.get('pinned-tabs.preventCloseBatch')) return;

        // Verify that all tabs are being closed and not just this one
        if (!this.willCloseBatch) return;

        // Prevent the PaneItem from being destroyed if it is pinned
        if(this.state.isPinned(pane.id, item)) prevent();
      })
    );
    this.subscriptions.add(
      atom.commands.onWillDispatch(({type}) => {
        if (BATCH_CLOSE_ACTIONS.includes(type)) this.willCloseBatch = true;
      })
    );
    this.subscriptions.add(
      atom.commands.onDidDispatch(({type}) => {
        if (BATCH_CLOSE_ACTIONS.includes(type)) this.willCloseBatch = false;
      })
    );
  }
