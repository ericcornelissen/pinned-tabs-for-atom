...
class PinnedTabsState {
  ...
  /**
   * Check if a PaneItem is currently pinned.
   *
   * @param {Number} paneId  The identifier of the Pane the `item` is in.
   * @param {Object} item    The PaneItem to check.
   */
  isPinned(paneId, item) {
    if (!this[paneId]) return;

    let itemId = getPaneItemId(item);
    let itemIndex = this[paneId].findIndex(item => item.id === itemId);
    return itemIndex >= 0;
  }
