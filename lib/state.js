...
class PinnedTabsState {
   ...
   *
   * @param {Pane} pane    The Pane instance of the pane for the item.
      ...
      if (item.onDidChangeTitle !== undefined) {
        stateItem.subscriptions.add(
          item.onDidChangeTitle(() => {
            let index = this[pane.id].findIndex(item => item.id === itemId);
            if (index >= 0) {
              setTimeout(() => { this[pane.id][index].id = getPaneItemId(item); });
            }
          })
        );
      }

      // Remove the item when the tab is destroyed
      if (item.onDidDestroy !== undefined) {
        stateItem.subscriptions.add(item.onDidDestroy(() => this.removePaneItem(pane, item)));
      }
