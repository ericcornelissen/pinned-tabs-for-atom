...
class PinnedTabs {
  ...
  activate(state={}) {
    ...
    // Add pin button to all initial tabs
    let tabs = document.querySelectorAll('.tab:not([data-type="TreeView"])');
    tabs.forEach(this.addPinButtonTo.bind(this));
  }
  ...
  deactivate() {
    ...
    // Remove pin button from all tabs
    let tabs = document.querySelectorAll('.tab');
    tabs.forEach(this.removePinButtonFrom);
  }
  ...
  observeConfig() {
    ...
    atom.config.observe('pinned-tabs.pinButton', enable => {
      body.classList.toggle('pinned-tabs-pin-button', enable);
    });
  ...
  setObservers() {
    ...
    this.subscriptions.add(
      atom.workspace.observePanes(pane => {
        ...
        // Add pin button to new tabs
        this.subscriptions.add(
          pane.onDidAddItem(({item}) => {
            let tab = findTabByItem(item, pane);
            this.addPinButtonTo(tab);
          })
        );
      })
    );
  ...
  /**
   * Add a button to pin the tab to a tab.
   *
   * @param {Node} tab  The tab of the item of interest.
   */
  addPinButtonTo(tab) {
    ...
    pinButton.addEventListener('click', () => this.pin(tab, true));

    tab.appendChild(pinButton);
  }

  /**
   * Remove the button to pin the tab from a tab.
   *
   * @param {Node} tab  The tab of the item of interest.
   */
  removePinButtonFrom(tab) {
    let pinButton = tab.querySelector('.pin-icon');
    if (pinButton !== null) tab.removeChild(pinButton);
  }
