...
const { findPaneItem, findTabById, getPaneItemId, isAncestor } = require('./util.js');
...
class PinnedTabs {
   ...
   */
  setObservers() {
    // Update the state for every new pane, and observe moving items
    this.subscriptions.add(
      atom.workspace.observePanes(pane => {
        ...
        this.subscriptions.add(
          ...
          pane.onDidMoveItem(({item, newIndex}) => setTimeout(() => this.reorderTab(pane, item, newIndex)))
        );
      })
    );

    // Remove a pined item from the state when the tab is destroyed
    this.subscriptions.add(
      atom.workspace.onDidDestroyPaneItem(({item, pane}) => this.state.removePaneItem(pane, item))
    );
    ...
    this.subscriptions.add(
      atom.workspace.onDidAddPaneItem(({index, item, pane}) => this.reorderTab(pane, item, index))
    );
  }
  ...
  reorderTab(pane, item, newIndex) {
    ...
    let tab = findTabById(id, pane);
    if (tab === undefined) return;

    let pinnedTabsCount = pane.element.querySelectorAll('.tab.pinned').length;
    if (this.isPinned(tab) && newIndex > pinnedTabsCount - 1) {
      pane.moveItem(item, pinnedTabsCount - 1);
