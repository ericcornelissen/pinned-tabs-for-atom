...
class PinnedTabs {
   ...
   *
   * @return {PinnedTabs} The new PinnedTabs.
   */
  constructor() {
   ...
   *
   * @param  {PinnedTabsState} state [description]
   */
  activate(state={}) {
   ...
   *
   * @return {Object} Serialized version of the PinnedTabs.
   */
  serialize() {
  ...
  initObservers() {
    ...
    atom.workspace.onWillDestroyPane(({item, pane}) => {
      if (!Array.isArray(this.state.data[pane.id])) {
        return;
      }

      this.state.data[pane.id] = this.state.data[pane.id].filter(id => id !== this.getItemID(item));
    });
  }

  /**
   * Initialize the tabs that should be pinned.
   */
  initTabs() {
    let activePane = atom.workspace.getActivePane();
    atom.workspace.getPanes().forEach(pane => {
      if (this.state.data[pane.id] === undefined) {
        return;
      }

      this.state.data[pane.id].forEach(target =>
  ...
  /**
   * Close all unpinned tabs within a pane in the Atom workspace.
   */
  closeUnpinned() {
  ...
  pin(item, tab) {
    ...
    if (this.isPinned(tab)) {
      ...
      if (!this.state.data[pane.id].includes(this.getItemID(item))) {
        this.state.data[pane.id].push(this.getItemID(item));
      }
      ...
      if (item.onDidChangeTitle !== undefined) {
        ...
        item.onDidChangeTitle(() => {
          if (!this.state.data[pane.id].includes(oldId)) {
            return;
          }

          this.state.data[pane.id] = this.state.data[pane.id].filter(id => id !== oldId);
   ...
   * @return {Boolean}     Indication of whether the tab is pinned.
   */
  isPinned(tab) {
